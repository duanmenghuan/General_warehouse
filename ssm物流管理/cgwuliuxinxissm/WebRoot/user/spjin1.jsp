...
var userId = <%=userId%>;
	...
	function shenheSpjin(){
		...
		$.messager.confirm("系统提示","您确认要验收吗？",function(r){
			if(r){
				$.post(url,{spjinType:2},function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						$("#dg").datagrid("reload");
					}else{
...
<!--startprint-->
	<table id="dg" title="货物运输" class="easyui-datagrid" fitColumns="true"
	 pagination="true" url="../getSpjins?spjinType=1&userId=<%=userId %>" fit="true" toolbar="#tb">
		<thead>
			<tr>
	...
	<div id="tb">
		<div>
			<a href="javascript:shenheSpjin()" class="easyui-linkbutton" iconCls="icon-edit" plain="true">验收</a>
		</div>
	</div>
<!--endprint-->
